...
map<int,map<LL,LL> > remember_answer; 
LL get_kth_of_mu(const int &n, const LL &k)
{
	if(remember_answer[n].count(k))
	return remember_answer[n][k];
	LL l = 0, r = k * 5;
	LL numl = 0;
	LL numr = cnt(n, r);
	while (l < r - 1)
	...
	return remember_answer[n][k]=r;
}

int main()
{
	freopen("thefall4.in", "r", stdin);
	freopen("thefall4.out", "w", stdout);
	prepare();
	int T;
	scanf("%d", &T);
	while (T--)
