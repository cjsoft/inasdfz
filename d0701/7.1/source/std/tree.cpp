...
#include<cstdio>
using namespace std;
const int mo=998244353,gg=3,N=503,lim=600;
const long long M=4ll*mo*mo;
...
void fft(int *x,int n,int fl=1){
    ...
    for (int i=0;i<n;i++) x[i]=(x[i]+mo)%mo;
}
int len=0,n;
    ...
    for (int i=1;i<n;i++){
    	...
    	for (int j=1;j<=i;j++){
    		...
    		for (int k=0;k<(1<<len);k++){
                long long k1=1ll*a[k]*c[k]%mo;
                if (x[k]>0) x[k]-=M;
                x[k]+=(1ll*(a[k]+b[k])*(c[k]+d[k])-k1);
                if (y[k]>0) y[k]-=M;
    			y[k]+=k1;
    		}
    	}
